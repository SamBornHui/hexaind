...
from app.services.data.curation.data_transformation.drop_columns.service import (
    DropColumnsService,
)
from app.services.workflows.designer.schemas import (
...
app = create_celery_app("drop_columns_worker")
logger = logging.getLogger(__package__)


# Testing
@app.task(name="task_drop_columns")
def task_drop_columns(action_id: str):
    logger.info("Inside task_drop_columns.")
    with common_widget_manager(app, action_id) as (
        ...
        if widget.type != WidgetType.DROP_COLUMNS:
...
def handle_drop_columns(
    ...
    data_trn_service = DropColumnsService()  # logic
    ...
    if widget_type == WidgetType.DROP_COLUMNS:
        dataset_metadata = DatasetMetadata(
            data_source=DatasetSourceFormats.GENERATED_IN_WORKFLOW_EXECUTION_FORMAT.format(
                widget.type.value.lower()
            )
        )
        drop_columns_config: DropColumnsConfig = widget.config.config

        file_ext = dataset_record.dataset_location[0].extension
        unique_file = f"DC_{widget.outputs[0].name}_{uuid.uuid4()}{file_ext}"  # file name to store the result
        output_file_path = str(Path(dest_file_path, unique_file))
        file_path = data_trn_service.drop_columns_handler(
            drop_columns_config=drop_columns_config,
