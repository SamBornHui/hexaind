...
from app.core.celery.global_config import LINEAR_REGRESSION_ACTION_WORKER_DEFAULT_QUEUE
...
app = create_celery_app('linear_regression_action_worker', default_queue=LINEAR_REGRESSION_ACTION_WORKER_DEFAULT_QUEUE)
logger = logging.getLogger(__package__)
@app.task(name="task_linear_regression")
def task_linear_regression(action_id: str):
    logger.info("inside task_linear_regression")
    with common_widget_manager(app, action_id) as (
        ...
        if widget.type != WidgetType.LINEAR_REGRESSION:
            logger.exception("Invalid action submited to LinearRegression worker")
            raise Exception("Invalid action submited to LinearRegression worker")
        ...
        if not prev_action_results or len(prev_action_results) > 1 or prev_action_results[0].result_type != ActionResultType.DATASET:
            logger.exception("LinearRegression action got non-compatible result(s) from it's prev action")
            raise Exception("LinearRegression action got non-compatible result(s) from it's prev action")
        ...
        if not dataset_path_info.isfolder:
            ...
            logger.exception("LinearRegression action got folder as a result from it's prev action.")
            raise Exception("LinearRegression action got folder as a result from it's prev action.")
        
        # initializing the linear-regression service
        linear_regression_config: LinearRegressionConfig = widget.config
        ...
        logger.info('LinearRegression kwargs:   ', kwargs)
        # calling RandomForest service class
        lr_service = LinearRegressionService(db_sync_client=action_handler.db_client)
        lr_output = lr_service.train_linear_regression(linear_regression_config, **kwargs)
        if lr_output.exception_detail:
            logger.exception(f"LinearRegression widget raised exception: {lr_output.exception_detail}")
            raise Exception(f"LinearRegression widget raised exception: {lr_output.exception_detail}")
        
        
        dataset_id = action_handler.datasets_handler.save_tabular_dataset_helper_sync(input_data=str(lr_output.tabular_path), project_id=run_record.project_id, site_id=run_record.site_id, user_id=run_record.owner_id,
        ...
        if len(widget.outputs) == 1:
            ...
            action_result_id = action_handler.create_action_result_record(ActionResult(type=ActionResultType.DATASET, result=DatasetActionResult(dataset_id=dataset_id), output_name=widget.outputs[0].name))
            action_result_id_2 = action_handler.create_action_result_record(ActionResult(type=ActionResultType.FILE_OR_FOLDER_PATH, result=FileActionResult(file_path_value=lr_output.models_path), output_name=widget.outputs[1].name))
            logger.info(f"created the action result record with action result id {action_result_id}")
