...
@Component({
  selector: 'app-nntorch-ml-config',
  templateUrl: './nntorch-ml-config.component.html',
  styleUrls: ['./nntorch-ml-config.component.less'],
})
export class NnTorchMlConfigComponent {
  ...
  config: NnTorchMLWidgetConfig | undefined = undefined;
  configCache: NnTorchMLWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget.config as NnTorchMLWidgetConfig;
    this.config.widget_type = WidgetType.NN_TORCH;
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'Neural Network (Torch) widget allows supported HEXAIND 3.0 users to train Neural Network Classification models with Pytorch backend, based on AutoGluon implementation';
      }
    }
