...
# **get_extra_links**
> ExtraLinkCollection get_extra_links(dag_id, dag_run_id, task_id, map_index=map_index)
...
with airflow_client.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("The response of TaskInstanceApi->get_mapped_task_instance_dependencies:\n")
        pprint(api_response)
    except Exception as e:
...
> TaskInstanceHistoryCollection get_mapped_task_instance_tries(dag_id, dag_run_id, task_id, map_index, limit=limit, offset=offset, order_by=order_by)
...
> TaskInstanceHistory get_mapped_task_instance_try_details(dag_id, dag_run_id, task_id, map_index, task_try_number)
...
with airflow_client.ApiClient(configuration) as api_client:
    ...
    executor = ['executor_example'] # List[str] | The value can be repeated to retrieve multiple matching values (OR condition). (optional)
    order_by = 'order_by_example' # str | The name of the field to order the results by. Prefix a field name with `-` to reverse the sort order.  *New in version 2.1.0*  (optional)
 ...
 **executor** | [**List[str]**](str.md)| The value can be repeated to retrieve multiple matching values (OR condition). | [optional] 
 **order_by** | **str**| The name of the field to order the results by. Prefix a field name with &#x60;-&#x60; to reverse the sort order.  *New in version 2.1.0*  | [optional] 
...
> TaskInstanceHistoryCollection get_task_instance_tries(dag_id, dag_run_id, task_id, limit=limit, offset=offset, order_by=order_by)
...
> TaskInstanceHistory get_task_instance_try_details(dag_id, dag_run_id, task_id, task_try_number)
...
with airflow_client.ApiClient(configuration) as api_client:
    ...
    task_id = 'task_id_example' # str | The task ID.
    map_index = 56 # int | The map index.
    update_task_instance = airflow_client.UpdateTaskInstance() # UpdateTaskInstance | Parameters of action (optional)
...
with airflow_client.ApiClient(configuration) as api_client:
    ...
    dag_run_id = 'dag_run_id_example' # str | The DAG run ID.
    task_id = 'task_id_example' # str | The task ID.
    update_task_instance = airflow_client.UpdateTaskInstance() # UpdateTaskInstance | Parameters of action
...
with airflow_client.ApiClient(configuration) as api_client:
    ...
    task_id = 'task_id_example' # str | The task ID.
    map_index = 56 # int | The map index.
    set_task_instance_note = airflow_client.SetTaskInstanceNote() # SetTaskInstanceNote | Parameters of set Task Instance note.
