...
**token_id** | **str** | The id of the token authorizing the request, if any. This id can be used to revoke the token via the API. Null if the request is not authorized with a token.  Added in 5.0.  | [optional] 
...
request_identity_from_dict = RequestIdentity.from_dict(request_identity_dict)
```
