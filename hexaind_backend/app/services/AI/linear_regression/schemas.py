...
class LinearRegressionConfig(BaseModel):
    ...
    widget_type: Literal[WidgetType.LINEAR_REGRESSION]
    
    problem_type: Optional[ProblemType] = Field(ProblemType.regression, description="problem type for linear regression")
    
    input_cols: List[str] = Field(..., description="input features for AutoML")
        
    output_col: str = Field(..., description="output feature for linear regression")
    
    input_scaling: Optional[InputScaling] = Field(default="Standard", description="Input Scaling for linear regression")

    split_ratio: int = Field(default= 80, description="test train split ratio for linear regression")

    evaluation_metric: EvaluationMetric = Field(default="root_mean_squared_error", description="Evaluation Metric for linear regression")

    lr_hyperparameters: LinearRegressionHyperparameters = Field(..., description="linear regression Hyperparameters")
    
    additional_hyperparameter_tune_kwargs: AdditionalHyperparameterTuneKWargs = Field(..., description="Additional linear regression Hyperparameters")
...
class LinearRegressionResponse(BaseModel):
    ...
    models_path: Optional[str] = Field(None, description="models result of LR widget")
