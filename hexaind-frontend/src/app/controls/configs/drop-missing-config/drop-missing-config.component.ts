...
import { DropMissingWidgetConfig, InputOutputConfig, Widget, WidgetType } from 'src/app/models/workflow-models';
...
@Component({
  selector: 'app-drop-missing-config',
  templateUrl: './drop-missing-config.component.html',
  styleUrls: ['./drop-missing-config.component.less']
})
export class DropMissingConfigComponent implements OnInit {
  ...
  projectId: any;
  sessionId: any;
  config: DropMissingWidgetConfig | undefined = undefined;
  configCache: DropMissingWidgetConfig | undefined = undefined;
  ...
  selectedColumnsDataSource!: MatTableDataSource<ColumnToSelect>;
  ...
  constructor(
    ...
    if (this.widgetControl) {
      this.config = this.widgetControl.Widget.config as DropMissingWidgetConfig;

      if (this.config) {
        this.config.widget_type = WidgetType.DROP_MISSING;
        ...
        if (this.config.config) {
          ...
          if (this.selectedList) {
            ...
            this.selectedColumnsDataSource = new MatTableDataSource(this.selectedColumnsList);
          }
        }
      }
  ...
  initializeInformation() {
    this.data = {
      ...
      description: 'Drop Missing allows users to drop rows with missing data',
      version: '1.0',
    };
  }
  ...
  toggleAllSelectedColumns(event: any) {
    const checked = event.checked;
    const filteredSelectedColumnsList = this.selectedColumnsDataSource.filteredData.slice();
  ...
  openColumnDropDown() {
    ...
    const dialogRef = this.dialog.open(ColumnDropDownComponent, {
      ...
      data: {
        ...
        widgetType: WidgetType.DROP_MISSING,
      },
    })
