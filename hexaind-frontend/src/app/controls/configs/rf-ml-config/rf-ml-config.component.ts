...
@Component({
  selector: 'app-rf-ml-config',
  templateUrl: './rf-ml-config.component.html',
  styleUrls: ['./rf-ml-config.component.less'],
})
export class RfMlConfigComponent {
  ...
  config: RandomForestRegressionMLWidgetConfig | undefined = undefined;
  configCache: RandomForestRegressionMLWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget
      .config as RandomForestRegressionMLWidgetConfig;
    this.config.widget_type = WidgetType.RF;
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'Random Forest widget allows supported HEXAIND 3.0 users to train Random Forest Classification models based on AutoGluon implementation';
      }
    }
