...
from .dao import RandomForestDao 
from .schemas import RandomForestConfig, RandomForestResponse
...
class RandomForestService:
    def __init__(self, db_sync_client: MongoClient = None, db_async_client: AsyncIOMotorClient = None) -> None: # type: ignore
        logger.info("Initializing RandomForestService")

        self.rf_dao = RandomForestDao(db_sync_client=db_sync_client, db_async_client=db_async_client)


    def send_notification(self, notification_obj):
        verified_notfication_obj = NotificationModel(**notification_obj)
        notification_service_obj = Notification(db_sync_client=self.rf_dao.db_sync_client)
        notification_service_obj.create_notification_sync(verified_notfication_obj)

    
    def train_rf(self, ml_config: RandomForestConfig, data_path: Path, result_folders: Path, project_id: str, wf_id: str, run_id: str, dataset_name: str, user_id: str, site_id: str, user_name: str, dataset_record: Dataset=None, widget_urn: str=None) -> RandomForestResponse:
        try:
            logger.info("Starting Random Forest training")
            logger.info(f"Random Forest configuration {str(ml_config)}")
            ...
            hyperparameters = ml_config.rf_hyperparameters.model_dump()
            ...
            experiment_name = f"RandomForest_{project_id}_{wf_id}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            mlflow.set_experiment(experiment_name)
            mlflow.set_tag("RandomForest version", "v1.0")
            mlflow.set_tag("Model type", "RandomForest")
            mlflow.set_tag('custom_run_id', run_id)
            flat_params_dict = ModelUtils.flatten_dict(ml_config.model_dump())
            notification_obj = {"message":"Random Forest Model training has started",
                                'category_id':wf_id,
            ...
            self.ml_model = dict(user_id=user_id, project_id=project_id, site_id=site_id, wf_run_id=run_id, description='RandomForest model description', created_at=datetime.datetime.now(),
            ...
            with mlflow.start_run(nested=True) as run:
                model_name = f"RandomForest_model_{run.info.run_id}"
                ...
                predictor = ModelUtils.train_and_log_models(data, train_data, test_data, hyperparameters, ml_config.evaluation_metric, ml_config.output_col, model_path, hyperparameter_tune_kwargs, problem_type, self.model_detail, self.extra_metrics, self.rf_dao.db_sync_client, self.ml_model)
                predictor.save()
            ...
            notification_obj['message'] = "Model Random Forest has been trained"
            self.send_notification(notification_obj=notification_obj)

            return RandomForestResponse(tabular_path=self.summary_file,models_path=os.path.join(artifact_final_path,"model"))

        
        except Exception as e:
            notification_obj = {"message":"Random Forest Model training has failed",
                                'category_id':wf_id,
                                'project_id':project_id,
            ...
            self.send_notification(notification_obj=notification_obj)
            return RandomForestResponse(exception_detail=str(e))
