...
from app.core.celery.global_config import MPR_ACTION_WORKER_DEFAULT_QUEUE
...
app = create_celery_app('MPR_action_worker', default_queue=MPR_ACTION_WORKER_DEFAULT_QUEUE)
logger = logging.getLogger(__package__)
@app.task(name="task_mpr")
def task_mpr(action_id: str):
    logger.info("inside task_mpr")
    with common_widget_manager(app, action_id) as (
        ...
        if not prev_action_results or len(prev_action_results) > 1 or prev_action_results[0].result_type != ActionResultType.DATASET:
            logger.exception("MPR action got non-compatible result(s) from it's prev action")
            raise Exception("MPR action got non-compatible result(s) from it's prev action")
        ...
        if not dataset_path_info.isfolder:
            ...
            logger.exception("MPR action got folder as a result from it's prev action.")
            raise Exception("MPR action got folder as a result from it's prev action.")
        
        # initializing the linear-regression service
        mpr_config: MPRConfig = widget.config
        ...
        logger.info('MPR kwargs:   ', kwargs)
        # calling RandomForest service class
        mpr_service = MPRService(db_sync_client=action_handler.db_client)
        mpr_output = mpr_service.train_mpr(mpr_config, **kwargs)
        if mpr_output.exception_detail:
            logger.exception(f"MPR widget raised exception: {mpr_output.exception_detail}")
            raise Exception(f"MPR widget raised exception: {mpr_output.exception_detail}")
        
        
        dataset_id = action_handler.datasets_handler.save_tabular_dataset_helper_sync(input_data=str(mpr_output.tabular_path), project_id=run_record.project_id,
                                                                                       site_id=run_record.site_id, user_id=run_record.owner_id,
                                                                                                        action_id=action_id, run_id=run_record.id, 
        ...
        if len(widget.outputs) == 1:
            ...
            action_result_id = action_handler.create_action_result_record(ActionResult(type=ActionResultType.DATASET, result=DatasetActionResult(dataset_id=dataset_id), output_name=widget.outputs[0].name))
            action_result_id_2 = action_handler.create_action_result_record(ActionResult(type=ActionResultType.FILE_OR_FOLDER_PATH, result=FileActionResult(file_path_value=mpr_output.models_path), output_name=widget.outputs[1].name))
            logger.info(f"created the action result record with action result id {action_result_id}")
