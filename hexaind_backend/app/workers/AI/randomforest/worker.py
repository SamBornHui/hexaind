...
from app.services.AI.randomforest.service import RandomForestService
from app.workers.utils import common_widget_manager
from app.core.celery.global_config import RF_ACTION_WORKER_DEFAULT_QUEUE
from app.core.celery.celery_worker import create_celery_app #create_celery_non_global_app #create_celery_app


app = create_celery_app('rf_action_worker', default_queue=RF_ACTION_WORKER_DEFAULT_QUEUE)
logger = logging.getLogger(__package__)


@app.task(name="task_rf")
def task_rf(action_id: str):
    logger.info("inside task_randomforest")
    ...
    with common_widget_manager(app, action_id) as (
        ...
        if widget.type != WidgetType.RF:
            logger.exception("Invalid action submited to RandomForest worker")
            raise Exception("Invalid action submited to RandomForest worker")
        ...
        if not prev_action_results or len(prev_action_results) > 1 or prev_action_results[0].result_type != ActionResultType.DATASET:
            logger.exception("RandomForest action got non-compatible result(s) from it's prev action")
            raise Exception("RandomForest action got non-compatible result(s) from it's prev action")
        ...
        if not dataset_path_info.isfolder:
            ...
            logger.exception("RandomForest action got folder as a result from it's prev action.")
            raise Exception("RandomForest action got folder as a result from it's prev action.")
        
        # initializing the automl service 
        rf_config: RandomForestConfig = widget.config
        ...
        logger.info('RandomForest kwargs:   ', kwargs)
        # calling RandomForest service class
        rf_service = RandomForestService(db_sync_client=action_handler.db_client)
        rf_output = rf_service.train_rf(rf_config, **kwargs)

        if rf_output.exception_detail:
            logger.exception(f"RandomForest widget raised exception: {rf_output.exception_detail}")
            raise Exception(f"RandomForest widget raised exception: {rf_output.exception_detail}")
        
        
        dataset_id = action_handler.datasets_handler.save_tabular_dataset_helper_sync(input_data=str(rf_output.tabular_path), project_id=run_record.project_id, site_id=run_record.site_id, user_id=run_record.owner_id, 
        ...
        if len(widget.outputs) == 1:
            ...
            action_result_id = action_handler.create_action_result_record(ActionResult(type=ActionResultType.DATASET, result=DatasetActionResult(dataset_id=dataset_id), output_name=widget.outputs[0].name))
            action_result_id_2 = action_handler.create_action_result_record(ActionResult(type=ActionResultType.FILE_OR_FOLDER_PATH, result=FileActionResult(file_path_value=rf_output.models_path), output_name=widget.outputs[1].name))
            logger.info(f"created the action result record with action result id {action_result_id}")
