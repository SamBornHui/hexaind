...
@Component({
  selector: 'app-nnr-ml-config',
  templateUrl: './nnr-ml-config.component.html',
  styleUrls: ['./nnr-ml-config.component.less'],
})
export class NnrMlConfigComponent {
  ...
  config: NeuralNetworkRegressionMLWidgetConfig | undefined = undefined;
  configCache: NeuralNetworkRegressionMLWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget
      .config as NeuralNetworkRegressionMLWidgetConfig;
    //this.config.widget_type = WidgetType;
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'Neural Network (FastAI) widget allows supported HEXAIND 3.0 users to train Neural Network Classification models with FastAI backend, based on AutoGluon implementation';
      }
    }
