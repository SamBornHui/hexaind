...
class BigQueryJobsManager:
    def __init__(
        ...
        self.gcp_project_id = service_credentials['authentication_details'].get("project_id")
        self.credentials_json = service_credentials['authentication_details']
        self.gcs_bucket_name = service_credentials['gcs_bucket_name']

        if not self.gcp_project_id or not self.credentials_json:
            raise ValueError(
                "Missing required BigQuery details: 'project_id' and 'credentials_json'."
            )
        ...
        self.bq_client = bigquery.Client(
            credentials=self.credentials, project=self.gcp_project_id
        )
        self.storage_client = storage.Client(
            project=self.gcp_project_id, credentials=self.credentials
        )
    ...
    def submit_query_and_export_to_gcs(
        ...
        unique_id = uuid.uuid4().hex #TODO add bucket name in the payload at connector creation
    ...
    def download_from_gcs(
        ...
        """
        try:
            destination_path = str(self.nfs_mount_path) + f"/p_{hexaind_project_id}/BigQuery_data_at_{time.strftime('%Y%m%d%H%M%S')}"
            ...
            for blob in blobs:
                if not blob.name.endswith(
                    ...
                    try:
                        ...
                        print("continuing with next file if its in queue")
                    finally:
