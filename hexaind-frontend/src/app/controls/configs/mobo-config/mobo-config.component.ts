...
import { MoboConfigService } from './mobo-config.service';
...
@Component({
  selector: 'app-mobo-config',
  templateUrl: './mobo-config.component.html',
  styleUrls: ['./mobo-config.component.less'],
})
export class MoboConfigComponent implements OnInit {
  ...
  config: MOBOWidgetConfig | undefined = undefined;
  configCache: MOBOWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget.config as MOBOWidgetConfig;
    this.config.widget_type = WidgetType.MOBO;
    ...
    if (
      ...
      if (this.inputWidgets.length > 0) {
        ...
        if (this.inputURN.length > 0) {
          ...
          if (configuration?.source.type === 'LOCAL') {
          ...
          } else if (configuration?.source.type === 'BIGQUERY') {
            this.getTheDatasetIDFromPreviousWidget();
          }
  ...
  async getTheDatasetIDFromPreviousWidget() {
    try {
      const resultDetails = await this.moboConfigService.getDatasetResults(
        this.site_id,
        this.project_id,
      ...
      if (resultDetails && resultDetails.length > 0) {
        const dataset_id = resultDetails[0]?.result_value?._id;
        if (dataset_id) {
      ...
      this.toaster.error(
        'Action Record not found for previous widget',
        'ERROR',
        {
  ...
  saveWorkflow() {
    ...
    this.configCache.output_variables_objectives = [];
    this.configCache.input_variables_constraints = [];
    this.configCache.output_variables_thresholds = [];
    ...
    for (var i = 0; i < selectedOutputs.length; i++) {
      ...
      if (selectedOutputs[i].min_max_value == 1) {
        this.configCache.output_variables_objectives.push('MINIMUM');
      } else {
        this.configCache.output_variables_objectives.push('MAXIMUM');
      }
      this.configCache.output_variables_thresholds.push(
        selectedOutputs[i].threshold,
      );
    ...
    this.configCache.outcome_constraints_variable =
      this.outcome_constraints_variable;
    this.configCache.outcome_constraints_operator = this.operator;
    ...
    this.configCache.outcome_constaints_inputvalue = this.inputValue;
    this.configCache.outcome_constraints_active =
      this.workflowDetail.outcome_constraints_active;
  }
  ...
  setWorkflowDetail() {
    ...
    this.inputValue = this.configCache.outcome_constaints_inputvalue;
    this.operator = this.configCache.outcome_constraints_operator;
    this.outcome_constraints_variable =
      this.configCache.outcome_constraints_variable;
    this.outcome_constraints_variable =
      this.configCache.outcome_constraints_variable;
  ...
  ValidateConfiguration() {
    ...
    if (
      ...
      for (var j = 0; j < selectedOutputs.length; j++) {
        if (
          selectedOutputs[j].threshold === '' ||
          selectedOutputs[j].threshold === null
        ) {
          outputFlag = true;
          this.validationCheck = false;
          this.toaster.error('Please fill all required fields', '', {
        ...
        if (
          selectedOutputs[j].min_max_value === '' ||
          selectedOutputs[j].min_max_value === null
        ) {
          outputFlag = true;
          this.validationCheck = false;
          this.toaster.error('Please fill all required fields', '', {
