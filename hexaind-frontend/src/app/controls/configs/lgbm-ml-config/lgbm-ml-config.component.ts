...
import {
  ...
  ProblemType,
  OptimizationMethod,
  LGBMMLWidgetConfigMLHyperparameters,
...
@Component({
  selector: 'app-lgbm-ml-config',
  templateUrl: './lgbm-ml-config.component.html',
  styleUrls: ['./lgbm-ml-config.component.less'],
})
export class LGBMMlConfigComponent {
  @ViewChild(SettingsComponent) settingsComponent!: SettingsComponent;
  selectedRun: WorkflowRun | undefined = undefined;
  changeMade: boolean = false;
  outputChangeMade: boolean = false;
  outputChangeMade2: boolean = false;
  inputChangeMade: boolean = false;
  selectedOption: string | undefined;
  changeSetting: boolean = false;
  config: LGBMMLWidgetConfig | undefined = undefined;
  configCache: LGBMMLWidgetConfig | undefined = undefined;
  widgetControl: WidgetControl | undefined;
  outputName: string | undefined = undefined;
  outputName2: string | undefined = undefined;
  outputName3: string | undefined = undefined;
  ...
  allInputChecked = false;
  allCatInputChecked = false;

  numFeatureSearch: string = '';
  catFeatureSearch: string = '';

  scalingMethods = Object.values(InputScaling).filter(
    (value) => typeof value === 'string',
  );
  selectedScalling = InputScaling.Standard;

  selectedOptimization = OptimizationMethod.GridSearch;
  optimizationMethod = Object.values(OptimizationMethod).filter(
    (value) => typeof value === 'string',
  );
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget.config as LGBMMLWidgetConfig;
    this.config.widget_type = WidgetType.LGBM;
    this.configCache = JSON.parse(JSON.stringify(this.config));
    this.configCache!.lgbm_hyperparameters = this.configCache!
      .lgbm_hyperparameters as LGBMMLWidgetConfigMLHyperparameters;
    ...
    if (this.widgetControl.Widget.outputs.length) {
      ...
      this.outputName3 = this.widgetControl.Widget.outputs[2]?.name;
    }
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'GBM widget allows supported HEXAIND 3.0 users to train Gradient Boosting Machine (GBM) Classification models, based on AutoGluon implementation';
      }
    }
  ...
  loadSelectedInputWidget() {
    ...
    if (this.configCache && this.configCache.input_scaling !== undefined) {
      this.selectedScalling = this.configCache.input_scaling;
    }

    if (
      this.configCache &&
      this.configCache.lgbm_hyperparameters?.GBM
        ?.Selected_Optimization_method !== undefined
    ) {
  ...
  get widgetOutput3(): string | undefined {
    return this.outputName3;
  }

  set widgetOutput3(value: string | undefined) {
    this.outputName3 = value;
    this.outputChangeMade = true;
  }
  ...
  onOutputSave() {
    ...
    if (this.outputName3) {
      this.outputName3 = this.outputName3.trim();
      if (
        this.widgetControl &&
        this.widgetControl.Widget.outputs &&
        this.widgetControl.Widget.outputs.length > 0
      ) {
        this.widgetControl.Widget.outputs[2].name = this.outputName3;
      }
    }
  ...
  storeSelectedInputScalling(event: any) {
    ...
    if (this.configCache) {
      this.configCache.input_scaling = this.selectedScalling;
      this.changeMade = true;
    }
  }

  storeSelectedOptimization(event: any) {
    this.selectedOptimization = event.value;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Selected_Optimization_method =
        this.selectedOptimization;
      if (
        this.configCache!.lgbm_hyperparameters!.GBM!
          .Selected_Optimization_method === OptimizationMethod.RandomSearch
      ) {
        if (this.config!.lgbm_hyperparameters!.GBM!.n_iter) {
          this.configCache!.lgbm_hyperparameters!.GBM!.n_iter =
            this.config!.lgbm_hyperparameters!.GBM!.n_iter;
        } else {
          this.configCache!.lgbm_hyperparameters!.GBM!.n_iter = 50;
          this.changeMade = true;
        }
      } else {
        this.configCache!.lgbm_hyperparameters!.GBM!.n_iter = undefined;
        this.changeMade = true;
      }
    }
  }
  ...
  setCv(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.cv = value;
      this.changeMade = true;
    }
  }
  getCv() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.cv;
  }

  setRandomMethod(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.random_state = value;
      this.changeMade = true;
    }
  }
  getRandomMethod() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.random_state;
  }
  ...
  isRandomSearch() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!
      .Selected_Optimization_method === OptimizationMethod.RandomSearch
      ? true
      : false;
  }

  setNoIteration(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.n_iter! = value;
      this.changeMade = true;
    }
  }
  getNoIteration() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.n_iter;
  }

  setRandomState(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.random_state = value;
      this.changeMade = true;
  ...
  setMinDepth(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Min_depth = value;
      this.changeMade = true;
    }
  }
  getMinDepth() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Min_depth;
  }

  setMaxDepth(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Max_depth = value;
      this.changeMade = true;
    }
  }
  getMaxDepth() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Max_depth;
  }

  setSampleNoDepth(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_depth = value;
      this.changeMade = true;
    }
  }
  getSampleNoDepth() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_depth;
  }

  setMinNumLeaves(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Min_num_leaves = value;
      this.changeMade = true;
    }
  }
  getMinNumLeaves() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Min_num_leaves;
  }

  setMaxNumLeaves(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Max_num_leaves = value;
      this.changeMade = true;
    }
  }
  getMaxNumLeaves() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Max_num_leaves;
  }
  setSampleNoLeaves(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_num_leaves = value;
      this.changeMade = true;
    }
  }
  getSampleNoLeaves() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_num_leaves;
  }

  setMinChildSamples(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Min_child_samples = value;
      this.changeMade = true;
    }
  }
  getMinChildSamples() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Min_child_samples;
  }

  setMaxChildSamples(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.Max_child_samples = value;
      this.changeMade = true;
    }
  }
  getMaxChildSamples() {
    ...
    return this.configCache!.lgbm_hyperparameters!.GBM!.Max_child_samples;
  }
  setSampleNoChildSamples(event: any) {
    let value = +event;
    if (this.configCache) {
      this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_child_samples =
        value;
      this.changeMade = true;
    }
  }
  getSampleNoChildSamples() {
    if (!this.configCache) {
      return undefined;
    }
    return this.configCache!.lgbm_hyperparameters!.GBM!.SampleNo_child_samples;
  }
