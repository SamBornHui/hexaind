...
from app.services.data.curation.data_transformation.drop_missing.service import (
    DropMissingService,
)
from app.services.workflows.designer.schemas import (
...
app = create_celery_app("drop_missing_worker")
logger = logging.getLogger(__package__)


# Testing
@app.task(name="task_drop_missing")
def task_drop_missing(action_id: str):
    logger.info("Inside task_drop_missing.")
    with common_widget_manager(app, action_id) as (
        ...
        if widget.type != WidgetType.DROP_MISSING:
...
def handle_drop_missing(
    ...
    data_trn_service = DropMissingService()  # logic
    dataset_service = DatasetsService(db_sync_client=db_client)  #
    logger.info("received the drop missing service and dataset service")
    ...
    if widget_type == WidgetType.DROP_MISSING:
        dataset_metadata = DatasetMetadata(
            data_source=DatasetSourceFormats.GENERATED_IN_WORKFLOW_EXECUTION_FORMAT.format(
                widget.type.value.lower()
            )
        )
        drop_config: DropMissingConfig = widget.config.config

        file_ext = dataset_record.dataset_location[0].extension
        unique_file = (
            f"DM_{widget.outputs[0].name}_{uuid.uuid4()}{file_ext}"  # file name to store the filter result
        )
        output_file_path = str(Path(dest_file_path, unique_file))

        file_path = data_trn_service.drop_missing_by_column_handler(
    ...
    if file_path != None:
        ...
        error_str = (
            f"Widget Type {widget_type} Cannot be handled by drop_missing worker"
        )
        logger.exception(error_str)
        raise Exception(error_str)
