...
from .dao import NNTorchDao 
from .schemas import NNTorchConfig, NNTorchResponse
...
class NNTorchService:
    def __init__(self, db_sync_client: MongoClient = None, db_async_client: AsyncIOMotorClient = None) -> None: # type: ignore
        logger.info("Initializing NN_Torch Service")
        self.nntorch = NNTorchDao(db_sync_client=db_sync_client, db_async_client=db_async_client)


    def send_notification(self, notification_obj):
        verified_notfication_obj = NotificationModel(**notification_obj)
        notification_service_obj = Notification(db_sync_client=self.nntorch.db_sync_client)
        notification_service_obj.create_notification_sync(verified_notfication_obj)

    
    
    def train_nntorch(self, ml_config: NNTorchConfig, data_path: Path, result_folders: Path, project_id: str, wf_id: str, run_id: str, dataset_name: str, user_id: str, site_id: str, user_name: str, dataset_record: Dataset=None, widget_urn: str=None) -> NNTorchResponse:
        try:
            logger.info("Starting NN_torch training")
            logger.info(f"NN torch configuration {str(ml_config)}")
            ...
            logger.info("Preprocessing for NN_torch")
            ...
            hyperparameters = ml_config.nntorch_hyperparameters.model_dump()
            ...
            experiment_name = f"NNTorch_{project_id}_{wf_id}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            mlflow.set_experiment(experiment_name)
            mlflow.set_tag("NNTorch version", "v1.0")
            mlflow.set_tag("Model type", "NNTorch")
            mlflow.set_tag('custom_run_id', run_id)
            flat_params_dict = ModelUtils.flatten_dict(ml_config.model_dump())
            notification_obj = {"message":"NN_Torch Model training has started",
                                'category_id':wf_id,
            ...
            self.ml_model = dict(user_id=user_id, project_id=project_id, site_id=site_id, wf_run_id=run_id, description='NNTorch model description', created_at=datetime.datetime.now(),
            ...
            with mlflow.start_run(nested=True) as run:
                model_name = f"NNTorch{run.info.run_id}"
                ...
                predictor = ModelUtils.train_and_log_models( data, train_data, test_data, hyperparameters, ml_config.evaluation_metric, ml_config.output_col, model_path, hyperparameter_tune_kwargs, problem_type, self.model_detail, self.extra_metrics, self.nntorch.db_sync_client, self.ml_model)
                predictor.save()
            ...
            notification_obj['message'] = "Model NN_Torch has been trained"
            self.send_notification(notification_obj=notification_obj)

            return NNTorchResponse(tabular_path=self.summary_file,models_path=os.path.join(artifact_final_path,"model"))

        
        except Exception as e:
            logger.error("NN_Torch training failed", exc_info=True)

            notification_obj = {"message":"NN_Torch Model training has failed",
                                'category_id':wf_id,
                                'project_id':project_id,
            ...
            self.send_notification(notification_obj=notification_obj)
            return NNTorchResponse(exception_detail=str(e))
