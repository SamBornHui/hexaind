...
from app.services.data.curation.data_transformation.datatype_conversion.service import DataTypeConversionService
from app.services.workflows.designer.schemas import (
...
app = create_celery_app("dtypeconversion_worker")
logger = logging.getLogger(__package__)


# Testing
@app.task(name="task_datatype_conversion")
def task_datatype_conversion(action_id: str):
    logger.info("Inside datatype_conversion.")
    with common_widget_manager(app, action_id) as (
        ...
        if widget.type != WidgetType.DATATYPE_CONVERSION:
            raise Exception("Invalid action submited to DataTransform worker")

        handle_datatype_conversion(
            widget,
            action_handler,
...
def handle_datatype_conversion(
    widget: Widget,
    ...
    data_trn_service = DataTypeConversionService()  # logic
    dataset_service = DatasetsService(db_sync_client=db_client)  #
    logger.info("received the data type conversion service and dataset service")
    ...
    if widget_type == WidgetType.DATATYPE_CONVERSION:
        datatype_conversion_config: DataTypeConversionConfig = widget.config.config
        ...
        file_ext = ".parquet"
        unique_file = f"DTC_{widget.outputs[0].name}_{uuid.uuid4()}{file_ext}" # file name to store the result
        output_file_path = str(Path(dest_file_path, unique_file))
       
        file_path = data_trn_service.datatype_conversion_handler(
            datatype_conversion_config=datatype_conversion_config,
    ...
    if file_path != None:
        ...
        error_str = (
            f"Widget Type {widget_type} Cannot be handled by datatype conversion worker"
        )
        logger.exception(error_str)
        raise Exception(error_str)