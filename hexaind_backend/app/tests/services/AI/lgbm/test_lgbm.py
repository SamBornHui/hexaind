...
def lgbm_service():
    ...
    return LGBMService(db_sync_client=db_sync_client, db_async_client=db_async_client)

def test_lgbm_service_init(lgbm_service):
    assert lgbm_service is not None


@pytest.mark.fixme
def test_train_lgbm(lgbm_service):
    config = dict(
    ...
    site_id = 'site_id'
    user_name = 'user_name'
    response = lgbm_service.train_lgbm(ml_config, data_path, result_folders, project_id, wf_id, run_id, dataset_name, user_id, site_id, user_name)
    # Check results
    assert response.tabular_path


@pytest.mark.fixme
def test_train_lgbm_failure(lgbm_service):
    ...
    site_id = 'site_id'
    user_name = 'user_name'
    response = lgbm_service.train_lgbm(None, data_path, result_folders, project_id, wf_id, run_id, dataset_name, user_id, site_id, user_name)
