# -*- coding: utf-8 -*-
"""
Created on Thu May  9 13:35:12 2024
...
def standardize_inputs_outputs(X,y):
    '''
    ...
    train_y=y_scaler.fit_transform(y)


    if sq_flag_y:
        train_y=y.squeeze()
    
    return x_scaler, y_scaler, train_x, train_y
...
def parity_plot(train_y, preds, labels):
    fig, axes = plt.subplots(1,train_y.shape[1], figsize=(7*train_y.shape[1], 7), constrained_layout=True)
    for k in range(train_y.shape[1]):
        ax = axes[k] if train_y.shape[1] > 1 else axes
        # ax.errorbar(train_y[:,k],mean[:,k],yerr=0.5*(upper[:,k] - lower[:,k]),fmt='o',capsize=2,color='tab:blue',label='train', alpha=0.2) 
        # ax.plot([np.min(train_y[:,k]),np.max(train_y[:,k])],
        #              [np.min(train_y[:,k]),np.max(train_y[:,k])],'k--',linewidth=3)
        ax.plot(train_y[:,k],train_y[:,k],'k')
        ax.plot(train_y[:,k],preds[:,k],'b.')
        
        ax.set_xlabel(r'$'+str(labels[k])+'$')
        ax.set_ylabel(r'$\^'+str(labels[k])+'$')