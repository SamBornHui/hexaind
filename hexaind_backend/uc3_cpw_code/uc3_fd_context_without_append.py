...
def backup_and_empty_directory(source_directory: str, backup_directory: str):
    ...
    backup_dest = backup_path / f"{source_path.name}_backup"
    shutil.copytree(source_path, backup_dest, dirs_exist_ok=True)
    print(f"Backup created at: {backup_dest}")
    for item in source_path.iterdir():
        if item.is_file():
            item.unlink()  # Remove files
        elif item.is_dir():
            shutil.rmtree(item)  # Remove directories
    print(f"Directory '{source_directory}' has been emptied.")
...
def hexaind_custom_widget_function(
    ...
    if response.status_code == 200:
        ...
        workflow_owner_id = "67650fef7d1ad6470071b112"  # "669d4716fa67d7b1d58bbbc3"  # "67650fef7d1ad6470071b112"
    ...
    stage_name = (
        "FD_CONTEXT"  # hard coding stage name as uc3 sigma here for this widget
    )
    ...
    if data_pull_prefrence != "Pull Data Only":
        ...
        # folder to append data to
        override_destination_path = (
            Path(results_dir).parent / f"fd_context/{current_run_id}_{uuid.uuid4()}"
        )
        override_destination_path.mkdir(parents=True, exist_ok=True)
        override_destination = str(override_destination_path)
        ...
        if data_pull_prefrence == "Append to Existing":
            ...
            backup_and_empty_directory(
                str(override_destination_path.parent),
                str(override_destination_path.parent.parent / "fd_context_backup"),
            )
    ...
    elif stage_name == "FD_CONTEXT":
        url = f"http://micron_apis_proxy:80/micron/data_catalog/fd/fd_context_new?data_pull_job_id={new_data_pull_job_id}&user_name={user_name}"
    else:
    ...
    task_micron_data_pull(
        data_pull_job_id,
        stage_job_id,
        user_name,
        pull_directly_to_hexaind_platform=True,
    ...
    if response.status_code == 200:
        key_map = {
            ...
            "PROBE_DATA_PULL": "probe_data",
            "FD_CONTEXT": "fd_contexts",
        }
        file_path = (
            response.json()
    ...
    if override_destination is not None:
        df1 = pd.read_parquet(Path(override_destination).parent)
        return df1, pd.DataFrame(
            [
                {
                    "appended_path": str(Path(override_destination).parent),
                    "original": file_path,
                }
            ]
        )
    ...
    #         elif ("probe" in f) and ("pivot" not in f):
    #             df2: pd.DataFrame = pd.read_parquet(f)
    #         if "pivot" in f:
    #             df3 = pd.read_parquet(f)
    #     except Exception as e:
    #         print(f"Found issue {e} while reading file {f}")
    return df1, pd.DataFrame([summary_df])
