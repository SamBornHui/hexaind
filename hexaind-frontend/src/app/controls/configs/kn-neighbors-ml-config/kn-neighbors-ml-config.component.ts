...
@Component({
  selector: 'app-kn-neighbors-ml-config',
  templateUrl: './kn-neighbors-ml-config.component.html',
  styleUrls: ['./kn-neighbors-ml-config.component.less'],
})
export class KnNeighborsMlConfigComponent {
  ...
  config: KNearestNeighborMLWidgetConfig | undefined = undefined;
  configCache: KNearestNeighborMLWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget
      .config as KNearestNeighborMLWidgetConfig;
    this.config.widget_type = WidgetType.KNEIGHBORS;
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'K-Nearest neighbors (KNN) widget allows supported HEXAIND 3.0 users to train K-Nearest neighbors (KNN) Classification models, based on AutoGluon implementation';
      }
    }
