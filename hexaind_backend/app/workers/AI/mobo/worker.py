...
from app.services.data.assets.datasets.schemas import Dataset, DatasetType, DatasetSubType, AccessMode, DatasetLocation, \
...
from app.core.celery.global_config import MOBO_ACTION_WORKER_DEFAULT_QUEUE
...
app = create_celery_app('mobo_action_worker', default_queue=MOBO_ACTION_WORKER_DEFAULT_QUEUE)

os.environ["OMP_NUM_THREADS"] = "1" # liniting the operations to a single thread, avoiding concurrency issues in certain scenarios.
# NOTE: Please don't move this variable to .env, since it will reduce performance of the whole application. For now we are seeing issue only in mobo worker.

logger = logging.getLogger(__package__)

@app.task(name="task_mobo")
def task_mobo(action_id: str):
    logger.info("inside task_mobo")
    ...
    with common_widget_manager(app, action_id) as (
        ...
        logger.info(f"Current run state: {custom_run_state}")

        if widget.type != WidgetType.MOBO:
            logger.exception("Invalid action submited to MOBO worker")
        ...
        # check the state of the mobo widget
        if action_handler.action_record.custom_run_state and action_handler.action_record.custom_run_state.total_invoke_count != 0: # executed previously
            ...
            # if state: triggering second time, load state, read results from cycle widget, update experiment, update the state, new recommendation on the data which is in state variable, check terimination critiria, emit decision also along with mobo result
            ...
            logger.info(f"fetched Previous action results")
            
            if not prev_action_results or len(prev_action_results) > 1 or prev_action_results[0].result_type != ActionResultType.DATASET:
                logger.exception("MOBO action got non-compatible result(s) from it's prev action")
                raise Exception("MOBO action got non-compatible result(s) from it's prev action")
            ...
            if dataset_record.dataset_type != DatasetType.TABULAR:
                logger.exception(f"MOBO action got non-compatible result(s) from it's prev action. Expected: TABULAR. But got: {dataset_record.dataset_type}")
                raise Exception("MOBO action got non-compatible result(s) from it's prev action. Expected: TABULAR. But got: ", dataset_record.dataset_type)
            ...
            logger.info(f"built kwargs for mobo service {kwargs}")
            ...
            if mobo_output.exception_detail:
                logger.exception(f"MOBO widget raised exception: {mobo_output.exception_detail}")
                raise Exception(f"MOBO widget raised exception: {mobo_output.exception_detail}")
            ...
            if mobo_output.terminate:
                ...
                logger.info(f"created action result record {action_id}")
                ...
                # saving the action results to the collection
                action_result_id = action_handler.create_action_result_record(ActionResult(type=ActionResultType.DATASET, result=DatasetActionResult(dataset_id=dataset_id), output_name=widget.outputs[0].name))
                logger.info("saving the action results to the collection")
                ...
                logger.info("Update the custom run state.")
           ...
           # mobo_run_state: CustomRunState = CustomRunState(total_invoke_count=1)
            ...
            if not dataset_path_info.isfolder:
                ...
                logger.exception("MOBO action got folder as a result from it's prev action.")
                raise Exception("MOBO action got folder as a result from it's prev action.")
            ...
            if mobo_output.terminate:
                logger.exception("MOBO widget sends teriminated signal without generating the first batch of recommendations")
                raise Exception("MOBO widget sends teriminated signal without generating the first batch of recommendations")
            ...
            logger.info("update the action run state")
