...
from .dao import KNNDao
from .schemas import KNNConfig, KNNResponse
...
class KNNService:
    def __init__(self, db_sync_client: MongoClient = None, db_async_client: AsyncIOMotorClient = None) -> None: # type: ignore
        logger.info("Initializing KNN Service")
        self.knn_dao = KNNDao(db_sync_client=db_sync_client, db_async_client=db_async_client)


    def send_notification(self, notification_obj):
        verified_notfication_obj = NotificationModel(**notification_obj)
        notification_service_obj = Notification(db_sync_client=self.knn_dao.db_sync_client)
        notification_service_obj.create_notification_sync(verified_notfication_obj)
    
    
    def train_knn(self, ml_config: KNNConfig, data_path: Path, result_folders: Path, project_id: str, wf_id: str, run_id: str, dataset_name: str, user_id: str, site_id: str, user_name: str,dataset_record: Dataset= None, widget_urn: str = None) -> KNNResponse:
        try:
            logger.info("Starting k-nearest neighbour training")
            logger.info(f"KNN configuration {str(ml_config)}")
            # Verify data path and load data using pandas
            if not data_path.exists():
                raise FileNotFoundError(f"Data file not found at path: {data_path}")
            logger.info("Preprocessing for KNN")
            ...
            hyperparameters = ml_config.knn_hyperparameters.model_dump()
            ...
            experiment_name = f"KNN_{project_id}_{wf_id}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            mlflow.set_experiment(experiment_name)
            mlflow.set_tag("KNN version", "v1.0")
            mlflow.set_tag("Model type", "KNN")
            mlflow.set_tag('custom_run_id', run_id)
            flat_params_dict = ModelUtils.flatten_dict(ml_config.model_dump())
            notification_obj = {"message":"K_Nearest_Neighbour Model training has started",
                                'category_id':wf_id,
            ...
            self.ml_model = dict(user_id=user_id, project_id=project_id, site_id=site_id, wf_run_id=run_id, description='KNN model description', created_at=datetime.datetime.now(),
            ...
            with mlflow.start_run(nested=True) as run:
                model_name = f"KNN_model_{run.info.run_id}"
                ...
                predictor = ModelUtils.train_and_log_models(data, train_data, test_data, hyperparameters, ml_config.evaluation_metric, ml_config.output_col, model_path, hyperparameter_tune_kwargs, problem_type, self.model_detail, self.extra_metrics, self.knn_dao.db_sync_client, self.ml_model)
                predictor.save()
            ...
            notification_obj['message'] = "Model K_Nearest_Neighbour has been trained"
            self.send_notification(notification_obj=notification_obj)

            return KNNResponse(tabular_path=self.summary_file,models_path=os.path.join(artifact_final_path,"model"))

        
        except Exception as e:
            logger.error("KNN training failed", exc_info=True)

            notification_obj = {"message":"K_Nearest_Neighbour Model training has failed",
                                'category_id':wf_id,
                                'project_id':project_id,
            ...
            self.send_notification(notification_obj=notification_obj)
            return KNNResponse(exception_detail=str(e))
