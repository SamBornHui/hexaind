...
@Component({
  selector: 'app-extra-trees-ml-config',
  templateUrl: './extra-trees-ml-config.component.html',
  styleUrls: ['./extra-trees-ml-config.component.less'],
})
export class ExtraTreesMlConfigComponent {
  ...
  config: ExtraTreeMLWidgetConfig | undefined = undefined;
  configCache: ExtraTreeMLWidgetConfig | undefined = undefined;
  ...
  constructor(
    ...
    this.config = this.widgetControl.Widget.config as ExtraTreeMLWidgetConfig;
    this.config.widget_type = WidgetType.EXTRA_TREES;
  ...
  loadFeatures() {
    ...
    if (this.configCache && this.configCache.problem_type) {
      if (Utils.isRegressionModel(this.configCache)) {
        ...
        description =
          'Extra Trees widget allows supported HEXAIND 3.0 users to train Extra Trees Classification models, based on AutoGluon implementation';
      }
    }
